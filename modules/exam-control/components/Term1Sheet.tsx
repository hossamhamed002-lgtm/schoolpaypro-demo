...
const Term1Sheet: React.FC<Term1SheetProps> = ({
  students,
  subjects,
  grades,
  filterGrade,
  ...
  const results = useMemo(() => {
    ...
    students.forEach((student) => {
      ...
      relevantSubjects.forEach((sub) => {
        const record = stGrades[sub.id]?.term1;
      ...
      <tr className="bg-white text-[8px]">
        {subjectsList.map((sub) => (
          <React.Fragment key={sub.id}>
            ...
            <th className="bg-blue-100 font-black w-9">جملة</th>
          </React.Fragment>
        ))}
      </tr>
    </thead>
        ...
        return (
          <React.Fragment key={`${student.id}-${includeStudentColumns ? 'main' : 'cont'}`}>
            <tr className="h-7 hover:bg-slate-50 odd:bg-white even:bg-slate-50/30">
              ...
              {subjectsList.map((sub) => {
                const rec = grades[student.id]?.[sub.id]?.term1;
                ...
                const totalCellClass =
                  resultView === 'scores'
                    ? `font-black bg-blue-100/50 ${isSubFail ? 'text-red-600' : ''}`
      ...
      {pages.length === 0 ? (
        ...
        pages.map((pageStudents, pIdx) => (
          <React.Fragment key={pIdx}>
            <div className="official-sheet-page" id={pIdx === 0 ? 'term1-sheet-print-area' : undefined}>
              <div className="flex justify-between items-center mb-4 border-b-4 border-black pb-2">
                ...
                <div className="text-center">
                  <h2 className="text-xl font-black">شيت رصد درجات الفصل الدراسي الأول</h2>

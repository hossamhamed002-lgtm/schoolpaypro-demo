
import React, { useMemo, useState } from 'react';
import { ArrowRight, Printer, FileDown, Loader2, FileSpreadsheet } from 'lucide-react';
import { Student, Subject, GradesDatabase, GradeLevel, GRADE_LABELS, SchoolInfo } from '../examControl.types';
import { db } from '../services/db';
import { exportUtils } from '../services/exportUtils';

interface Props {
  students: Student[];
  subjects: Subject[];
  grades: GradesDatabase;
  onBack: () => void;
}

type StatCell = { boys: number; girls: number; total: number };
interface GradeStatRow {
    gradeLabel: string;
    registered: StatCell;
    present: StatCell;
    passedR1: StatCell;
    passedR2: StatCell;
    totalPassed: StatCell;
    failedRepeat: StatCell;
    percentage: number;
}

const MiddleCombinedRoundsOfficialStats: React.FC<Props> = ({ students, subjects, grades, onBack }) => {
  const [selectedGrade, setSelectedGrade] = useState<GradeLevel | 'all_prep'>('all_prep');
  const [isExporting, setIsExporting] = useState(false);
  const schoolInfo = db.getSchoolInfo();
  const certConfig = db.getCertConfig();

  const middleGrades: GradeLevel[] = ['m1', 'm2'];

  const stats = useMemo(() => {
    const targetGrades = selectedGrade === 'all_prep' ? middleGrades : [selectedGrade as GradeLevel];
    const PASS_THRESHOLD = certConfig.minPassingPercent / 100;
    const EXAM_THRESHOLD = certConfig.minExamPassingPercent / 100;

    return targetGrades.map(g => {
        const row: GradeStatRow = {
            gradeLabel: GRADE_LABELS[g],
            registered: { boys: 0, girls: 0, total: 0 },
            present: { boys: 0, girls: 0, total: 0 },
            passedR1: { boys: 0, girls: 0, total: 0 },
            passedR2: { boys: 0, girls: 0, total: 0 },
            totalPassed: { boys: 0, girls: 0, total: 0 },
            failedRepeat: { boys: 0, girls: 0, total: 0 },
            percentage: 0
        };

        const gradeStudents = students.filter(s => s.gradeLevel === g);
        const gradeSubjects = subjects.filter(s => s.gradeLevels?.includes(g) && s.isBasic);

        gradeStudents.forEach(student => {
            const isBoy = student.gender === 'ذكر';
            const stGrades = grades[student.id] || {};
            row.registered.total++; if (isBoy) row.registered.boys++; else row.registered.girls++;
            
            let attendedAny = false;
            let failedRound1 = false;
            let subjectsToRetake: string[] = [];

            // 1. تحديد من رسب في الدور الأول
            gradeSubjects.forEach(sub => {
                const rec = stGrades[sub.id];
                const safeVal = (v: any) => (v === undefined || v === -1) ? 0 : v;
                const t1 = safeVal(rec?.term1?.work) + safeVal(rec?.term1?.practical) + safeVal(rec?.term1?.exam);
                const t2 = safeVal(rec?.term2?.work) + safeVal(rec?.term2?.practical) + safeVal(rec?.term2?.exam);
                const annualAvg = (t1 + t2) / 2;
                const t2_exam = safeVal(rec?.term2?.exam);
                const passedWritten = (sub.examScore === 0) || (t2_exam >= sub.examScore * EXAM_THRESHOLD);

                if (rec?.term1?.exam !== -1 || rec?.term2?.exam !== -1) attendedAny = true;
                
                const subFailedR1 = (sub.examScore > 0 && (rec?.term1?.exam === -1 || rec?.term2?.exam === -1)) ||
                                   (!passedWritten) || (annualAvg < sub.maxScore * PASS_THRESHOLD);

                if (subFailedR1) {
                    failedRound1 = true;
                    subjectsToRetake.push(sub.id);
                }
            });

            // 2. فحص نتيجة الدور الثاني للراسبين
            let passedRound2 = false;
            if (failedRound1) {
                let hasAnyFailInR2 = false;
                subjectsToRetake.forEach(subId => {
                    const sub = subjects.find(s => s.id === subId);
                    const r2Rec = stGrades[subId]?.secondRole;
                    // إذا لم يدخل الامتحان أو لم يحقق النهاية الصغرى
                    if (!r2Rec || r2Rec.exam === -1 || r2Rec.exam < (sub?.minScore || 0)) {
                        hasAnyFailInR2 = true;
                    }
                });
                if (subjectsToRetake.length > 0 && !hasAnyFailInR2) passedRound2 = true;
            }

            if (attendedAny) {
                row.present.total++; if (isBoy) row.present.boys++; else row.present.girls++;
                
                if (!failedRound1) {
                    row.passedR1.total++; if (isBoy) row.passedR1.boys++; else row.passedR1.girls++;
                    row.totalPassed.total++; if (isBoy) row.totalPassed.boys++; else row.totalPassed.girls++;
                } else if (passedRound2) {
                    row.passedR2.total++; if (isBoy) row.passedR2.boys++; else row.passedR2.girls++;
                    row.totalPassed.total++; if (isBoy) row.totalPassed.boys++; else row.totalPassed.girls++;
                } else {
                    // طالب دخل الدور الثاني ولم ينجح -> باق للإعادة
                    row.failedRepeat.total++; if (isBoy) row.failedRepeat.boys++; else row.failedRepeat.girls++;
                }
            } else {
                // غياب كامل في الدورين -> باق للإعادة
                row.failedRepeat.total++; if (isBoy) row.failedRepeat.boys++; else row.failedRepeat.girls++;
            }
        });
        
        row.percentage = row.present.total > 0 ? (row.totalPassed.total / row.present.total) * 100 : 0;
        return row;
    });
  }, [students, subjects, grades, selectedGrade, certConfig]);

  const handleExportPDF = () => {
    setIsExporting(true);
    exportUtils.exportToPDF('middle-official-combined-area', 'إحصاء_الدورين_إعدادي_رسمي').then(() => setIsExporting(false));
  };

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border no-print">
        <div className="flex items-center gap-3">
          <button onClick={onBack} className="p-2 hover:bg-gray-100 rounded-lg text-gray-500 transition-colors"><ArrowRight size={20} /></button>
          <h2 className="text-xl font-bold">بيان إحصائي ختامي للدورين (رسمي)</h2>
        </div>
        <div className="flex gap-2">
            <select value={selectedGrade} onChange={e => setSelectedGrade(e.target.value as any)} className="border rounded px-3 py-1.5 font-bold text-blue-700 bg-gray-50 outline-none">
                <option value="all_prep">كل الصفوف (الأول والثاني)</option><option value="m1">أولى إعدادي</option><option value="m2">ثانية إعدادي</option>
            </select>
            <button onClick={() => exportUtils.exportTableToExcel('middle-combined-table', 'إحصاء_الدورين_إعدادي')} className="bg-emerald-50 text-emerald-700 px-4 py-2 rounded-lg border border-emerald-200 font-bold flex items-center gap-2"><FileSpreadsheet size={18}/> Excel</button>
            <button onClick={handleExportPDF} disabled={isExporting} className="bg-red-50 text-red-700 px-4 py-2 rounded-lg border border-red-200 font-bold flex items-center gap-2">{isExporting ? <Loader2 className="animate-spin" size={18}/> : <FileDown size={18}/>} PDF</button>
            <button onClick={() => exportUtils.print('middle-official-combined-area')} className="bg-gray-800 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2"><Printer size={18}/> طباعة</button>
        </div>
      </div>

      <div id="middle-official-combined-area" className="bg-white p-10 rounded border-2 border-black overflow-x-auto min-w-max text-black font-sans" dir="rtl">
          <div className="text-center mb-6 border-b-2 border-black pb-4">
              <h1 className="text-2xl font-black mb-1 text-black">{schoolInfo.schoolName}</h1>
              <h2 className="text-xl font-black underline decoration-2 underline-offset-8 text-black">بيان إحصائى مجمع لنتائج الدورين (الأول والثاني) - للعام الدراسى {schoolInfo.academicYear}</h2>
              <p className="text-sm font-bold mt-4">يشمل الموقف النهائي للطلاب (ناجح / باقٍ للإعادة)</p>
          </div>
          <table id="middle-combined-table" className="w-full text-center border-collapse border-2 border-black font-bold text-[12px]">
              <thead>
                  <tr className="bg-gray-100 h-12">
                      <th rowSpan={2} className="border-2 border-black p-1 w-8">م</th>
                      <th rowSpan={2} className="border-2 border-black p-2 bg-yellow-50 min-w-[140px]">الصف الدراسي</th>
                      <th colSpan={3} className="border-2 border-black p-1">متقدمون</th>
                      <th colSpan={3} className="border-2 border-black p-1">حاضرون</th>
                      <th colSpan={3} className="border-2 border-black p-1">ناجح دور اول</th>
                      <th colSpan={3} className="border-2 border-black p-1">ناجح دور ثان</th>
                      <th colSpan={3} className="border-2 border-black p-1 bg-gray-50 font-black">إجمالي الناجحين</th>
                      <th colSpan={3} className="border-2 border-black p-1 text-red-700 bg-red-50">راسب وباق للإعادة</th>
                      <th rowSpan={2} className="border-2 border-black p-1 bg-gray-200 w-16">النسبة %</th>
                  </tr>
                  <tr className="bg-white text-[10px]">
                      {Array.from({length: 6}).map((_, i) => (<React.Fragment key={i}><th className="border-2 border-black p-1">بنين</th><th className="border-2 border-black p-1">بنات</th><th className="border-2 border-black p-1 bg-gray-50">جملة</th></React.Fragment>))}
                  </tr>
              </thead>
              <tbody>
                  {stats.map((row, idx) => (
                      <tr key={idx} className="h-12 hover:bg-gray-50 transition-colors text-[13px]">
                          <td className="border-2 border-black">{idx + 1}</td>
                          <td className="border-2 border-black text-center px-2 bg-yellow-50/30 font-black">{row.gradeLabel}</td>
                          <td className="border-2 border-black">{row.registered.boys}</td><td className="border-2 border-black">{row.registered.girls}</td><td className="border-2 border-black bg-gray-50">{row.registered.total}</td>
                          <td className="border-2 border-black">{row.present.boys}</td><td className="border-2 border-black">{row.present.girls}</td><td className="border-2 border-black bg-gray-50">{row.present.total}</td>
                          <td className="border-2 border-black">{row.passedR1.boys}</td><td className="border-2 border-black">{row.passedR1.girls}</td><td className="border-2 border-black bg-gray-50">{row.passedR1.total}</td>
                          <td className="border-2 border-black">{row.passedR2.boys}</td><td className="border-2 border-black">{row.passedR2.girls}</td><td className="border-2 border-black bg-gray-50">{row.passedR2.total}</td>
                          <td className="border-2 border-black text-blue-700 font-black">{row.totalPassed.boys}</td><td className="border-2 border-black text-blue-700 font-black">{row.totalPassed.girls}</td><td className="border-2 border-black bg-blue-50 text-blue-800 font-black">{row.totalPassed.total}</td>
                          <td className="border-2 border-black text-red-600">{row.failedRepeat.boys}</td><td className="border-2 border-black text-red-600">{row.failedRepeat.girls}</td><td className="border-2 border-black bg-red-50 text-red-700 font-black">{row.failedRepeat.total}</td>
                          <td className="border-2 border-black font-black bg-white">%{row.percentage.toFixed(1)}</td>
                      </tr>
                  ))}
              </tbody>
          </table>
          <div className="mt-12 flex justify-between items-end px-4 font-black text-[13px] text-black">
              <div className="text-center w-44"><p className="mb-10">مسئول الحاسب</p><p className="border-t border-black pt-1">{schoolInfo.itSpecialist || '.................'}</p></div>
              <div className="text-center w-44"><p className="mb-10">رئيس الكنترول</p><p className="border-t border-black pt-1">{schoolInfo.controlHead || '.................'}</p></div>
              <div className="text-center w-44"><p className="mb-10">وكيل المدرسة</p><p className="border-t border-black pt-1">{schoolInfo.agentName || '.................'}</p></div>
              <div className="text-center w-44"><p className="mb-10">المدير التنفيذى</p><p className="border-t border-black pt-1">{schoolInfo.managerName || '.................'}</p></div>
          </div>
      </div>
    </div>
  );
};
export default MiddleCombinedRoundsOfficialStats;
